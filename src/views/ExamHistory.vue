<template>
    <div class="history-container">
      <el-card header="📋 套卷历史记录">
        <div style="margin-bottom: 15px; display: flex; gap: 10px;">
          <el-input v-model="searchPaperName" placeholder="输入试卷名称搜索..." style="width: 300px" @keyup.enter="loadData" clearable />
        ...
        <el-table :data="tableData" border style="width: 100%">
              ...
              <div style="padding: 10px 50px;">
                ...
                <el-table :data="props.row.moduleDetails" size="small" border>
                  ...
                  <el-table-column label="模块正确率" width="200">
                    <template #default="scope">
                      <span>{{ scope.row.totalCount > 0 ? ((scope.row.correctCount / scope.row.totalCount) * 100).toFixed(1) + '%' : '-' }}</span>
                    </template>
                  </el-table-column>
          ...
          <el-table-column label="正确率" width="120" align="center">
            <template #default="scope">
              <el-tag 
                :type="parseFloat(calculateOverallRate(scope.row.moduleDetails)) < 65 ? 'danger' : 'success'" 
  ...
  const loadData = async () => {
    const listRes = await getExamList({
      page: page.value,
      size: size.value,
